...
# The text search configuration of the keyword index (migration 0003): the
# French stemmer, after accents are folded. Part of the schema for the same
# reason as the dimension above - the generated column is computed with it,
# and a query parsed with any other configuration would match nothing.
FULL_TEXT_CONFIG = "french_unaccent"
...
class DocumentChunk(Base):
    ...
    # Generated by PostgreSQL from `text`, never written by the application:
    # the keyword half of hybrid retrieval (ADR-0015).
    text_tsv: Mapped[str] = mapped_column(
        TSVECTOR, Computed(f"to_tsvector({FULL_TEXT_CONFIG}::regconfig, text)", persisted=True)
    )
    ...
    __table_args__ = (
        ...
        Index("ix_document_chunks_text_tsv", "text_tsv", postgresql_using="gin"),
    )
